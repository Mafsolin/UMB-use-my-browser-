...
describe("SessionManager", () => {
  ...
  it("rejects side-effect actions when permission is false", () => {
    ...
    expect(() => manager.assertAllowed(session.sessionId, "submitForm")).toThrow(
      /side effects are disabled/i
    );
  });
  ...
  it("rejects navigation when navigation permission is false", () => {
    ...
    expect(() => manager.assertAllowed(session.sessionId, "navigate")).toThrow(
      /navigation is disabled/i
    );
  });
  ...
  it("rejects typing when typing permission is false", () => {
    ...
    expect(() => manager.assertAllowed(session.sessionId, "type")).toThrow(
      /typing is disabled/i
    );
  });
});
